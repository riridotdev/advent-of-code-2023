...
func main() {
	...
	for y := range grid {
		for x := range grid[y] {
			...
			distanceCount += totalDistance(grid, x, y, pairDistances, expandRows, expandCols, 1_000_000)
		}
	}
...
func totalDistance(grid [][]byte,
	startX, startY int,
	results GalaxyDistances,
	expandRows, expandCols []int,
	expandFactor int,
) int {
	totalDistance := 0
	steps := 1
			...
			for _, delta := range deltas {
				...
				if next != '.' {
					...
					for _, expandRow := range expandRows {
						if (expandRow > startY && expandRow < newY) ||
							(expandRow < startY && expandRow > newY) {
							result += expandFactor - 1
						}
					}
					for _, expandCol := range expandCols {
						if (expandCol > startX && expandCol < newX) ||
							(expandCol < startX && expandCol > newX) {
							result += expandFactor - 1
						}
					}
					results[resultsIdx] = result
					totalDistance += results[resultsIdx]
				}
