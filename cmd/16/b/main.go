...
func main() {
	...
	maxIlluminated := math.MinInt

	for x := range grid[0] {
		maxIlluminated = max(maxIlluminated, countIlluminated(grid, beam{x, -1, South}))
		maxIlluminated = max(maxIlluminated, countIlluminated(grid, beam{x, len(grid), North}))
	}

	for y := range grid {
		maxIlluminated = max(maxIlluminated, countIlluminated(grid, beam{-1, y, East}))
		maxIlluminated = max(maxIlluminated, countIlluminated(grid, beam{len(grid[0]), y, West}))
	}

	println(maxIlluminated)
}

func countIlluminated(grid [][]byte, start beam) int {
